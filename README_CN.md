...
AI Factory 是一个 Spring Boot 服务，编排 AI 驱动的任务拆解流水线。它通过 MCP 协议集成 GitNexus（第三方代码知识图谱服务）获取符号级代码上下文，并调用 OpenAI 兼容的 LLM（如 DashScope/通义千问）完成需求拆解、对话澄清和代码生成。

> **注意：** GitNexus 是第三方项目，不包含在本仓库中。你需要自行部署运行 GitNexus，并在 `application.yml` 中配置其 MCP 端点 URL。
...
- Java 17+
- MySQL 8+（或使用 Docker）
- 一个已索引你代码库的 GitNexus 实例（第三方项目，需自行部署）
...
"GitNexus" 是其各自所有者的商标或产品名称。AI Factory 是一个独立项目，通过 MCP 协议与 GitNexus 集成。AI Factory 不隶属于 GitNexus 项目或其维护者，也未获得其认可或赞助。所有商标均归各自所有者所有。
